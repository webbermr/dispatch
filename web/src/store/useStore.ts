...
interface DispatchState {
  ...
  addAppOpen: boolean
  pairCode: string
  health: { codexInstalled: boolean; codexVersion: string | null; ghInstalled: boolean; ghAuthed: boolean } | null
  ...
  newCard: () => void
  editCard: (id: string, patch: { title?: string; desc?: string; prompt?: string }) => void
...
const pendingPatch: Record<string, { title?: string; desc?: string; prompt?: string }> = {}
...
export const useStore = create<DispatchState>((set, get) => {
  ...
  const dispatchLive = async (id: string) => {
    ...
    if (!c) return
    try {
      const { runId, branch } = await agent.dispatch({ appId: c.appId, cardId: c.id, prompt: c.prompt ?? '', type: c.type, baseBranch: app(c.appId)?.base, title: c.title })
      updateCard(id, (cc) => ({ ...cc, status: 'building', runId, branch, build: { progress: 0, currentStep: 'Queued', logs: [] } }))
      set({ draggingId: null })
      toast(`Dispatched to Codex · ${branch}`)
    } catch (err) {
  ...
  return {
    ...
    init: async () => {
      ...
      try {
        ...
        set({ agentPresent: true, health: { codexInstalled: health.codexInstalled, codexVersion: health.codexVersion, ghInstalled: health.ghInstalled, ghAuthed: health.ghAuthed } })
        if (agent.token) {
          try {
    ...
    newCard: () => {
      ...
      if (get().live) {
        agent
          .createCard({ appId, title: 'Untitled card', desc: 'Describe what you want built, then edit the agent instructions before you dispatch.', prompt: 'Describe the feature, bug fix, or enhancement here.\n\n- What should change?\n- Any files or screens involved?\n- How will we know it works?' })
          .then((c) => {
      ...
      const newC: Card = {
        ...
        status: 'ideas',
        title: 'Untitled card',
        desc: 'Describe what you want built, then edit the agent instructions before you dispatch.',
        prompt:
          'Describe the feature, bug fix, or enhancement here.\n\n- What should change?\n- Any files or screens involved?\n- How will we know it works?',
      }
      set((s) => ({ cards: [newC, ...s.cards], openCardId: id }))
    },
    editCard: (id, patch) => {
    ...
    reconnect: () => {
      ...
      const tryReal = async (): Promise<boolean> => {
        ...
        try {
          ...
          set({ agentPresent: true, health: { codexInstalled: h.codexInstalled, codexVersion: h.codexVersion, ghInstalled: h.ghInstalled, ghAuthed: h.ghAuthed } })
          return true
        } catch {
          return false
        }
